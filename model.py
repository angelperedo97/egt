...
class GraphTransformerNet(nn.Module):
    def __init__(self, net_params):
        ...
        self.out_proj = nn.Linear(hidden_dim, hidden_dim)  # Embedding output
    ...
    def forward(self, g, h):
        ...
        embedding = self.out_proj(graph_embedding)  # Shape: [batch_size, hidden_dim]
        return embedding
...
class AffinityPredictionModel(nn.Module):
    def __init__(self, net_params):
        super().__init__()
        self.protein_transformer = GraphTransformerNet(net_params)
        self.ligand_transformer = GraphTransformerNet(net_params)
        hidden_dim = net_params['hidden_dim']

        # Define a fully connected layer to combine embeddings
        self.fc1 = nn.Linear(hidden_dim * 2, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, 1)  # Final output for regression

        self.dropout = nn.Dropout(net_params['dropout'])
        self.activation = nn.ReLU()

    def forward(self, receptor_graph, receptor_features, ligand_graph, ligand_features):
        # Obtain embeddings from both transformers
        protein_embedding = self.protein_transformer(receptor_graph, receptor_features)  # [batch_size, hidden_dim]
        ligand_embedding = self.ligand_transformer(ligand_graph, ligand_features)        # [batch_size, hidden_dim]

        # Concatenate embeddings
        combined_embedding = torch.cat([protein_embedding, ligand_embedding], dim=1)    # [batch_size, hidden_dim*2]

        # Pass through fully connected layers
        x = self.fc1(combined_embedding)                                               # [batch_size, hidden_dim]
        x = self.activation(x)
        x = self.dropout(x)
