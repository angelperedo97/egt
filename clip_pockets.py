...
def elem(t):
    ...
    name = names[t]
    if 'Hydrogen' in name:
        return None  # Skip hydrogen atoms
    elif 'Carbon' in name:
        return 'C'
...
def process_item(item, processed_pairs, pair_lock):
    ...
    - Save the clipped receptor and ligand as PDB files.
    ...
    try:
        ...
        receptor_base, _ = os.path.splitext(receptor_filename)
        ligand_base, _ = os.path.splitext(ligand_filename)
        
        # Ensure the clipped receptor filename has a .pdb extension
        clipped_receptor_filename = f"{receptor_base}_clipped_{ligand_base}.pdb"
        clipped_receptor_path = os.path.join(receptor_dir, clipped_receptor_filename)

        # Ensure the ligand filename has a .pdb extension
        ligand_pdb_filename = f"{ligand_base}.pdb"
        ligand_pdb_path = os.path.join(receptor_dir, ligand_pdb_filename)
        ...
        with pair_lock:
            if pair_key in processed_pairs:
                ...
                # Save clipped receptor and ligand as PDB files
                save_as_pdb(clipped_receptor_path, clipped_receptor_atoms)
                save_as_pdb(ligand_pdb_path, ligand_atoms)
        ...
        ligand_rel_path = os.path.relpath(ligand_pdb_path, DATA_DIR)
